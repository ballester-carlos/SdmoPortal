$(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            if (result.success) {
                                ...
                                $('#MessageToClient').text('The webserver had an error');
                            }
                        },
                        error: function () {
                            $('#MessageToClient').text('The webserver had an error');
    ...
    $('#partsModal').on('show.bs.modal', function () { recalculatePart(); });
    $('#partsModal').on('shown.bs.modal', function () {
        $('#InventoryItemCode').focus();
    });
    $('#partsModal').on('hide.bs.modal', function () { window.location.reload(); });

    $('#laborsModal').on('show.bs.modal', function () { recalculateLabor(); });
    $('#laborsModal').on('shown.bs.modal', function () {
        $('#ServiceItemCode').focus();
    });
    $('#laborsModal').on('hide.bs.modal', function () { window.location.reload(); });
});
...
function recalculatePart() {
    ...
    if (document.getElementById('UnitPrice') == null) {
        return;
    }
    */
...
function recalculateLabor() {
    ...
    var laborHours = parseFloat(document.getElementById('LaborHours').value).toFixed(0);
